...
class TaskController extends Controller
{
    ...
    public function worker_click_task_info(Request $request){
        ...
        }elseif($member->package->package_id == 3){

            $task_price_rate = 5;

        }elseif($member->package->package_id == 4){
        ...
        if (!empty($worker->parent_id) && !empty($worker->parent_user_code)) {
            ...
            if(!empty($first_parent->parent_id) && !empty($first_parent->parent_user_code)){
